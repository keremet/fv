...
if (isset($_GET['acc_id'])) {
	...
	$stmt = $db->prepare(
		"SELECT uch_id, DATE_FORMAT(creat_date, '%d%m%Y') as creat_date
                ,DATE_FORMAT(clos_date, '%d%m%Y') as clos_date, remark
		 FROM ofv_acc
		 WHERE id = ?");
...
<?php if ($acc_id!=null) { ?>
<input value="Удалить счёт" type="submit" onclick="return checkDelLoanAgr();">
